...
export default function HackathonsPage() {
  ...
  useEffect(() => {
    ...
    const fetchOpportunities = async () => {
      const { data, error } = await supabaseClient.from('opportunities').select('*').eq('type', 'hackathon');
      if (!cancelled) {
        if (!error && data) {
      ...
      <main className="max-w-7xl mx-auto px-4 py-24 sm:px-6 lg:px-8 mt-8">
        ...
        >
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-5 w-full flex flex-col items-center gap-4">
            ...
            <div className="w-full">
              <FilterBar filters={filters} onChange={setFilters} availableFilters={['beginnerFriendly', 'remote']} />
            </div>
        ...
        <section>
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <SectionHeader
              ...
              action={<Link href="/post-hackathon" className="inline-flex items-center justify-center rounded-xl bg-indigo-500 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-400 transition-colors shadow-[0_0_15px_rgba(99,102,241,0.2)]">Post Hackathon</Link>}
            />
          </motion.div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm font-medium text-zinc-300 mb-1">No hackathons found</p>
