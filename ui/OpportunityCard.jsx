...
function getDeadlineUrgency(dateStr) {
  ...
  if (diff <= 7)
    return "text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-950/50 border-red-100 dark:border-red-800/60";
  if (diff <= 21)
    return "text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/50 border-amber-100 dark:border-amber-800/60";
    ...
    <div className="group relative flex flex-col bg-white dark:bg-gray-800/80 border border-gray-200 dark:border-gray-700/60 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg dark:hover:shadow-gray-900/40 transition-all duration-300 hover:-translate-y-0.5">
      ...
      <div className="px-5 py-3.5 border-t border-gray-100 dark:border-gray-700/60 flex items-center justify-between">
        <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
          <span
            className={`flex items-center gap-1 px-2 py-0.5 rounded-md border font-medium ${deadlineClass}`}
          >
            <Calendar size={10} />
            {deadlineLabel}
          </span>
        ...
        <button
          className={`flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold text-white bg-gradient-to-r ${config.gradient} rounded-lg hover:opacity-90 hover:shadow-sm transition-all`}
        >
          Apply
